...
const ReviewsList: React.FC = () => {
  ...
  const fetchReviews = async () => {
    ...
    try {
      ...
      const transformedReviews = await Promise.allSettled(
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center">
      {loading && <Spinner size="medium" borderColor="rgba(239, 229, 255, 0.5)" />}
      {!loading && (
        <div className="reviews-list-container w-full h-full p-4 sm:p-8 bg-white flex flex-col">
          <h2 className="text-xl sm:text-2xl md:text-4xl font-semibold text-black mb-4 sm:mb-6">
            Ratings & Reviews
          </h2>
          <div className="flex justify-center items-center mb-4 sm:mb-6">
            <div className="total-reviews-container flex flex-col items-center bg-white p-4 rounded-lg shadow-lg sm:shadow-md md:shadow-lg w-24 h-24 sm:w-32 sm:h-32 md:w-40 md:h-40">
              <h3 className="text-base sm:text-lg md:text-xl font-bold mb-1 sm:mb-2 md:mb-3 text-center md:text-left">
                Total Reviews
              </h3>
              <span className="text-3xl sm:text-4xl md:text-6xl font-bold text-purple-600">
                {totalReviews}
              </span>
            </div>
          </div>
          ...
          <div className="flex w-full items-center justify-center mt-4 sm:mt-6">
            <Pagination>
              <PaginationContent>
                ...
                {Array(Math.ceil(totalReviews / take))
                  .fill('')
                  .map((_, index) => (
