...
export const WalletModal: FC<WalletModalProps> = ({ className = '', container = 'body' }) => {
    ...
    const [installedWallets, otherWallets, solflareWallet] = useMemo(() => {
        ...
        return [installed, [...loadable, ...notDetected], solflare];
    }, [wallets]);
    ...
    const getStartedWallet = useMemo(() => {
        return installedWallets.length
            ? // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
              installedWallets[0]!
            : wallets.find((wallet: { adapter: { name: WalletName } }) => wallet.adapter.name === 'Solflare') ||
    ...
    const handleWalletClick = useCallback(
        (event: MouseEvent, walletName: WalletName) => {
            select(walletName);
            handleClose(event);
            console.log("walletName-----------",getStartedWallet);
        },
        [select, handleClose]
    );
                            ...
                            <>
                                ...
                                <ul className="wallet-adapter-modal-list">
                                    <WalletListItem
                                        ...
                                        handleClick={(event) => handleWalletClick(event, solflareWallet[0].adapter.name)}
